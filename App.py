...
st_autorefresh = st.experimental_rerun if int(time.time()) % 120 == 0 else lambda: None
...
def suggest_strategy(pcr):
    if pcr > 1.2:
        return "🔽 High PCR – Bearish outlook. Consider Bear Call Spread, Long Puts."
    elif pcr < 0.8:
        return "🔼 Low PCR – Bullish outlook. Consider Bull Put Spread, Long Calls."
    else:
